...
export default function Partners() {
  ...
  const { partners, addPartner, isStorageReady } = useTipContext();
  ...
  const handleAddPartner = async () => {
    ...
    try {
      const created = addPartner(newPartnerName.trim());

      if (!created) {
        toast({
          title: "Partner already exists",
          description: `${newPartnerName.trim()} is already in your roster`,
          variant: "destructive"
        });
        return;
      }
      ...
      toast({
        title: "Partner added",
        description: `${created.name} has been added to partners`,
      });
    } catch (error) {
      console.error(error);
      toast({
